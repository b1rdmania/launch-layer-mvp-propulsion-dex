...
import { RaiseData } from '@/services/api';
...
const RaiseCard: React.FC<RaiseCardProps> = ({ raise }) => {
  ...
  const progress = parseFloat(totalAcceptedTokenRaised) / parseFloat(maxAcceptedTokenRaise) * 100;
  ...
  const getTimeIndicator = () => {
    ...
    if (status === 'upcoming') {
      ...
      const minutes = Math.floor((timeToPublic % (1000 * 60 * 60)) / (1000 * 60));
  ...
  return (
    <div className="bg-cradle-surface rounded-xl p-4 border border-cradle-surface-light hover:scale-[1.03] transition-transform duration-200 shadow-lg">
      ...
      <div className="mb-4">
        ...
        <p className="text-sm mt-2 text-cradle-text-secondary">{metadata.description}</p>
      </div>
