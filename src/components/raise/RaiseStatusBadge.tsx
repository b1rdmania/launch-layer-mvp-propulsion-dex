...
import { RaiseStatus } from '@/services/api';
...
const RaiseStatusBadge: React.FC<RaiseStatusBadgeProps> = ({ status, className }) => {
  ...
  return (
    <div className={cn('px-2 py-0.5 text-xs rounded-full font-medium inline-block', bgColor, className)}>
      {label}
    </div>
  );
};
